...
- **Undoing a publish**: feed posts get a clock button (admins, published, not reposts) that opens an "Unpublish and schedule" dialog in `feed.py` — it *requires* a future time, because an unpublished post with `is_scheduled=False` matches neither branch of `feed_visibility()` and would vanish for admins too. Published flow items get an undo button in `exercise_detail.py` that goes back to pending instead (the flow list still shows them); it keeps `scheduled_at` only if it is still in the future and clears it otherwise, else `publish_due_posts()` would republish it on the next feed load. Both set `published_at = None`. Participants' `feed_visibility()` also hides reposts whose original is unpublished, or the quote card would leak it. The per-row ▶ publish in the flow goes through a confirm dialog; "Publish next" does not.
- **Scenario flow drag-and-drop**: rows in `exercise_detail.py` carry `class=flow-row` and `data-id`, and `FLOW_DRAG_JS` (module-level constant, injected once per page via `ui.add_body_html`) delegates HTML5 drag events from `document` — per-row handlers would die on the first `load_flow()` rebuild. Drops emit the `flow_reorder` event (`ui.on`), which renumbers `sort_order` 1..n gap-free; the JS also moves the row in the DOM immediately so the drop feels instant, with the server re-render fixing the `#N` labels. Drag starting on a button is cancelled so the arrow/edit/delete buttons still work. Drop-indicator styles live in `static/theme.css` (`.flow-drop-above/.flow-drop-below`). The up/down arrows are kept for single-step moves.
- **Scenario flow re-renders keep the scroll position**: `load_flow()` in `exercise_detail.py` reads `window.scrollY` before `flow_container.clear()` and restores it after the rebuild (`restore_scroll()`). Emptying the container shrinks the document, so the browser clamps the scroll offset and used to dump the admin at the bottom of the page after every move/edit. Only the initial page-build call passes `preserve_scroll=False` (no client to query yet). The flow row's action cluster needs `shrink-0` — without it the flex row squeezes the buttons on top of the preview text — and the move arrows are filled discs (`unelevated color=grey-4 text-color=grey-10`), since flat grey arrows were invisible against the row. Rows missing a first/last arrow render a `w-8 h-8` spacer so the icons stay in line.
- **Dialog inputs go stale on reopen**: NiceGUI's input component writes the browser's own value back into the element's props when the dialog unmounts, so setting `.value` to something the server already held (or clearing it) does *not* repaint the field — the old text stays visible while the server sees the new value. Openers that preload a dialog (`open_edit_flow`, `open_edit_social`, `open_edit_news`) call `field.run_method("updateValue")` to force the sync; the "Publish at" fields in the *create* dialogs are deliberately **not** cleared after adding, both because admins add runs of injects at the same time and because clearing them server-side only desynced the two and silently dropped the time when the admin left the field untouched.
- **Editing posts**: edit dialogs (social + news in `feed.py`, scenario-flow items in `exercise_detail.py`) can also replace or remove the attached image — the dialog preloads `post.image_url` into a preview, a "Replace image" upload swaps it, "Remove image" clears it, and save writes `image_url` back (None when cleared). Old media files are left on disk.
- **Exercise states**: `draft` → `live` → `ended` → `archived`. Only `live` carries behaviour: the feed's 10s poll timer (`feed.py`), which is also what publishes due scheduled posts without a page reload. A `ready` state existed once and was removed — it only changed a badge colour; `startup()` folds any leftover `ready` rows back to `draft` (the Postgres enum label stays behind, unused). "Go Live" and "Back to Draft" (live → draft, reversible, nothing is unpublished) are both behind confirm dialogs.
- **Go viral**: admins boost a social post by setting `Post.boosted_at`; the feed orders `boosted_at desc nullslast`, then the effective feed timestamp (below), so boosted posts pin to the top with a highlight + "Viral" badge.
- **Feed ordering and paging** (`load_posts()` in `feed.py`): posts sort by `coalesce(published_at, scheduled_at, created_at) desc`, *not* `published_at` alone — pending scheduled posts have no `published_at`, so ordering on it alone collapsed every inject onto NULL and made their order (and which ones survived the page limit) arbitrary. Each feed renders `PAGE_SIZE` (20) posts with a "Load more" button that bumps that feed's entry in the `shown_limit` dict; the limit exists to keep NiceGUI's WebSocket payloads small, so raise the page size rather than removing it. `poll_for_changes()` counts with the same visibility predicate as `load_posts()`, otherwise admins get no refresh when pending injects change.
...
| `Exercise` | `exercises` | A simulation exercise with state machine (draft/live/ended/archived) |
